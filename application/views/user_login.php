...
<?php if($this->session->userdata('login_data')): ?>
    ...
    <?php if ($login['user_role'] == 'admin') { ?>
        <?php redirect('admin','refresh'); ?>
    <?php } else { ?>
        <?php redirect('user/'.url_title($login['username'], 'dash', true),'refresh'); ?>
     <?php } ?>
    ...
    <div class="login-right loginform font-set left">
        <div class="main-form login-form data-wrap">
            <div class="login-div">               
                ...
                <form id="login_form" method="post" action="<?php echo base_url() ?>/home/validation">
                    <div class="login-container">
					    					
                        <input placeholder="Enter your email" class="form-control" type="email" id="email" name="user_email" required="">
                        
                    </div>
					<input required="" placeholder="Enter your Password" class="nonull lpassword"  type="password" id="password" name="user_password">
                    
					<button class="btn btnlogin" type="submit">Login</button>
					<a class="forgot to-forgot link-change" href="">Forgot my Password ?</a>
                </form>
            </div>
            <p class="next-link">Not a Member Yet! <a class="link-change to-registration" href="<?php echo base_url() ?>/register" title="Sign Up">Join Us</a> it's Free.</p>
            <p class="next-link">Didnt Recieve Email Verification<a class="link-change to-registration" href="<?php echo base_url() ?>/resend-email-verification" title="Resend">Resend</a> it's Free.</p>
        </div>
